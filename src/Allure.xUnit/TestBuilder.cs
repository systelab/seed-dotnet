...
namespace Allure.builder
{
    public class Test 
    {
        ...
        public enum Fixture { BeforeFeature, BeforeScenario, AfterFeature, AfterScenario }
            ...
            if (test.listLinks != null)
            {
                if (test.listLinks.Count > 0)
                {
                    ...
                    for (int i = 0; i < test.listLinks.Count; i++)
                    {
                        ...
                        {
                            tr.links.Add(Link.Issue(name: test.listLinks[i].name, url: test.listLinks[i].url));
                        }
                        else
                        {
                            tr.links.Add(Link.Tms(name: test.listLinks[i].name, url: test.listLinks[i].url));
                        }
                    }
                }
            }
